...
contract NexusYieldManager is INexusYieldManager, SatoshiOwnable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    /// @notice The address of the BorrowerOperation.
    ...
    function initialize(
        ...
        address borrowerOperationAddr_
        ...
        borrowerOperations = IBorrowerOperations(borrowerOperationAddr_);
    }
        ...
        returns (uint256)
    {
        ...
        borrowerOperations.mint(receiver, debtTokenToMint);

        // mint debtToken fee to rewardManager
        if (fee != 0) {
            borrowerOperations.mint(address(this), fee);
        ...
        returns (uint256)
    {
        ...
        borrowerOperations.burn(msg.sender, amount);
        ...
        returns (uint256)
    {
        ...
        // mint debtToken to receiver
        borrowerOperations.mint(receiver, actualTransferAmtInUSD);
    ...
    function scheduleSwapOut(address asset, uint256 amount) external isActive nonReentrant returns (uint256) {
        ...
        borrowerOperations.burn(msg.sender, swapAmount);
