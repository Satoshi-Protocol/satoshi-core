...
contract TroveHelper is ITroveHelper {
    ...
    function getNicrListByTime(ITroveManager troveManager, address[] memory _borrower, uint256 time) external view returns (uint256[] memory) {
        require(time >= block.timestamp, "TroveHelper: invalid time");
    ...
    function calculateInterestIndexByTime(ITroveManager troveManager, uint256 time) external view returns (uint256 currentInterestIndex, uint256 interestFactor) {
        require(time >= block.timestamp, "TroveHelper: invalid time");
        (currentInterestIndex, interestFactor) = _calculateInterestIndex(troveManager, time);
    }
  
    // SortedTroves 
    function getNode(address sortedTrovesAddress, address _borrower) external view returns (bool exist, address nextId, address prevId) {
        ...
        returns (uint256, uint256)
    {
        ...
        (uint256 pendingCollateralReward, uint256 pendingDebtReward) = troveManager.getPendingCollAndDebtRewards(_borrower);
    ...
    function _calculateInterestIndex(ITroveManager troveManager, uint256 time) internal view returns (uint256 currentInterestIndex, uint256 interestFactor) {
