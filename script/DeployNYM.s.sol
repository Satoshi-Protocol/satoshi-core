...
import {
    ...
    MAX_PRICE,
    MIN_PRICE,
    BORROWER_OPERATIONS_PROXY_ADDRESS
...
contract DeployNYMScript is Script {
    ...
    function run() public {
        ...
        INexusYieldManager nexusYieldImpl = INexusYieldManager(0xE58550377c1BdBCd524F3E677340589C09B0FEB5); // new NexusYieldManager();

        bytes memory data = abi.encodeCall(
            INexusYieldManager.initialize,
            (ISatoshiCore(SATOSHI_CORE_ADDRESS), DEBT_TOKEN_ADDRESS, REWARD_MANAGER_PROXY_ADDRESS, BORROWER_OPERATIONS_PROXY_ADDRESS)
        );
    ...
    function _setAssetConfig() internal {
        ...
        IBorrowerOperations(BORROWER_OPERATIONS_PROXY_ADDRESS).rely(address(nym));
        IRewardManager(REWARD_MANAGER_PROXY_ADDRESS).setWhitelistCaller(address(nym), true);
        nym.setPrivileged(0x2acfb3F0255793c29A9aab335E5D77d0261B886B, true);
    }
}
