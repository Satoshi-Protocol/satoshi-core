...
contract LiquidateCollDecimals8Test is Test, DeployBase, TroveBase, TestConfig, Events {
    ...
    function test_LiquidateICRLessThan100InRecoveryMode() public {
        ...
        _openTrove(user2, 1e8, 10000e18);
        _openTrove(user3, 1e8, 10000e18);
        ...
        assertEq(collateral.balanceOf(satoshiCore.rewardManager()), vars.collGasCompensation / 2);
    ...
    function test_LiquidateSPNotEnoughInNormalMode() public {
        LiquidationVars memory vars;
        // open troves
        _openTrove(user1, 1000e8, 10000e18);
        _openTrove(user2, 1e8, 10000e18);
        _openTrove(user3, 1e8, 10000e18);
    ...
    function test_LiquidateICRLargeThanMCRInRecoveryMode() public {
        ...
        // scale to decimals 8
        vars.collToSendToSP = (debt1 * MCR) / price / 1e10;
        ...
        // check user4 gets the reward for liquidation
        assertEq(collateral.balanceOf(user4), vars.collGasCompensation / 2);
        assertApproxEqAbs(collateral.balanceOf(satoshiCore.rewardManager()), vars.collGasCompensation / 2, 1);
        assertEq(debtTokenProxy.balanceOf(user4), vars.debtGasCompensation);
    }
    ...
    function test_liquidateTroves_2ICRLessThan100InRecoveryMode() public {
        ...
        vm.startPrank(user5);
        assertEq(collateral.balanceOf(user5), 0);
        // redistibute the collateral and debt
        ...
        assertEq(collateral.balanceOf(user5), vars.collGasCompensation);
        assertEq(collateral.balanceOf(satoshiCore.rewardManager()), vars.collGasCompensation);
        ...
        console.log(collateral.balanceOf(user5));
    }
    ...
    function test_Liquidate2ICRLessThan100InRecoveryMode() public {
        ...
        // check user5 gets the reward for liquidation
        assertEq(collateral.balanceOf(user5), vars.collGasCompensation / 2);
        assertEq(collateral.balanceOf(satoshiCore.rewardManager()), vars.collGasCompensation / 2);
